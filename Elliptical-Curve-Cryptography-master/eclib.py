...
class EC(object):
    ...
    #     raise Exception("Invalid order")
        ...
        # if not self.is_valid(g) or g == self.zero:
        #     raise ValueError("Invalid point")

        order_factors = []
        order = 1
        for prime_factor, exponent in sympy.factorint(self.q).items():
            order_factor = self.mul(g, self.q // prime_factor)
            for j in range(1, exponent + 1):
                if self.mul(order_factor, self.q // (prime_factor ** j)) == self.zero:
                    order_factor = self.mul(order_factor, prime_factor)
            order_factors.append(prime_factor ** exponent)
            order *= prime_factor ** exponent
